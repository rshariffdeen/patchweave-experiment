...
if __name__ == "__main__":
    ...
    if (len(args) > 3):
        ...
        for i in ids:
            # ret = system(test_cmd)
            ret = subprocess.call([ "timeout 12s " + cur_dir + "/cli/wavpack " + test_dir + "/" + i + ".wav -o output 1>" + test_dir + "/" + i + ".out  2>" + test_dir + "/" + i + ".err"],
                                  shell=True);
            ...
            with open(test_dir + "/" + i + ".err", "r") as err_file:
                ...
                special_word = "runtime error: shift overflow: "
                # print(data)
